...
df = pd.read_csv("./list/list_of_qi.txt").values.tolist()
...
def threshold_calculator():
    ...
    trust = input('Is the receiver of the data a trusted organization? Yes/No: ').lower()
    linkage_possibility = input('Is our data such that public resources might have related information? Yes/No: ').lower()
    ...
    inputs = [trust.lower(), linkage_possibility.lower(), working_period]
    error = False
    if trust in ['yes', 'no'] and linkage_possibility in ['yes', 'no'] and working_period in range(1, 6):
...
def find_qi(df):
    ...
    for header in headers:
        for qi in QI_LIST:
            if header.lower().replace('"', '').replace("'", '').replace('-', '').replace('_', '').replace(' ', '').find(qi.lower()) != -1:
